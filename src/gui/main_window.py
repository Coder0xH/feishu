...
from src.gui.gui_components import SourceGroupFrame, TargetGroupFrame, ControlButtonsFrame
...
class FeishuGUI(QMainWindow):
    ...
    def add_source_group(self):
        ...
        if group_name and group_name not in self.source_frame.get_source_groups():
            ...
            # 更新目标群配置的源群列表
            self.target_frame.update_source_groups(self.source_frame.get_source_groups())
            self.save_config()
    ...
    def start_monitor(self):
        ...
        # 检查是否配置了目标群
        if not any(len(group_mapping_config.get_target_groups(sg)) > 0 
